...
class CollisionHelper{
   ...
   CollisionHelper(CollisionHelper old){
     this(old.id,old.foundhit,old.inside,old.collisionframe,old.intersectPoint,old.surfaceNormal, old.HitObjectMaterial, old.Center, old.Size, old.type, old.axis);
   }
   CollisionHelper(int _id, boolean _foundhit,boolean _inside, float _collisionframe, PVector _intPoint, PVector _SNormal, rMaterial _rMat, PVector _Center, PVector _Size, int _type, int _axis){
     id = _id;
     foundhit = _foundhit;
     inside = _inside;
